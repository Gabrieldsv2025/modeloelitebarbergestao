...
SET valor_comissao = ROUND((iv.subtotal * ch.percentual_comissao) / 100.0, 2)
FROM public.itens_venda iv
WHERE ch.venda_id = iv.venda_id 
  AND ch.item_id = iv.item_id
  AND ch.id = comissoes_historico.id;

-- Verificar se a correção foi aplicada corretamente
-- Esta query mostra a diferença entre valores antigos e novos para validação
SELECT 
  ch.barbeiro_id,
